	   ...
	   void **pg_store, int *perm_store)
{
	...
	if (req->req_omode & O_CREAT) {
		if ((r = file_create(path, &f, req->req_omode & O_MKDIR)) < 0) {
...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	(r = openfile_lookup(envid, req->req_fileid, &o)) >= 0 &&
		(r = file_read(o->o_file, ret->ret_buf, req->req_n, o->o_fd->fd_offset)) >= 0 &&
		(o->o_fd->fd_offset += r); // Update the seek position
	return r;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	(r = openfile_lookup(envid, req->req_fileid, &o)) >= 0 &&
		(r = file_write(o->o_file, req->req_buf, req->req_n, o->o_fd->fd_offset)) >= 0 &&
		(o->o_fd->fd_offset += r); // Update the seek position
	return r;
}
